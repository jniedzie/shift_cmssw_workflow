...
SUBMISSION_VARIABLES=(
	...
	DEBUG_MUON_PRIMARIES
	DEBUG_MUON_HITS
	DEBUG_MUON_TRACKING
	TRACE_PRIMARY_MUON_PATHS
	...
	SHIFT_LSS_FIELD_DATA_DIRECTORY
	...
	SHIFT_USE_MATERIAL_AWARE_VERTEX_TRANSPORT
	SHIFT_USE_FORWARD_COMMON_VERTEX_FIT
...
case "$SHIFT_USE_FORWARD_COMMON_VERTEX_FIT" in
    0|1) ;;
    *) echo "Invalid SHIFT_USE_FORWARD_COMMON_VERTEX_FIT: expected 0 or 1" >&2; exit 2 ;;
esac
if [[ "$SHIFT_USE_FORWARD_COMMON_VERTEX_FIT" == 1 ]]; then
    [[ "$SHIFT_USE_VERTEX_CONSTRAINED_REFIT" == 1 && "$SHIFT_TARGET_DETAILED_MATERIAL" == 1 ]] || {
        echo "Forward common-vertex fit requires vertex refit and detailed target material" >&2; exit 2;
    }
fi
...
for variable_name in CONDOR_REQUEST_CPUS CONDOR_REQUEST_MEMORY_MB; do
