...
def main():
    ...
    while True:
        ...
        if counts == [args.chunks]*2 and logs == [args.chunks]*2 and traces == args.chunks:
            break
        if any(n > args.chunks for n in counts+logs+[traces]):
    ...
    command = [sys.executable, str(Path(__file__).with_name("compare_lss_reconstruction.py")),
               ...
               "--expected-events",str(args.events),"--transport-directory",str(args.comparison/"logs")]
